...
Run them in order (00 → 03) — each stage loads the previous stage's saved
...
- The original "Financial_Transactions_Data_Analysis" notebook (now in
  `_old/`) had solid per-table EDA that never got carried into this
  rebuild's account-level view — worth mining for additional features
  (e.g. transaction counterparty bank diversity).
